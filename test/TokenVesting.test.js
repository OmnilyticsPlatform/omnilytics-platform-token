...
contract("TokenVesting", function([_, owner, beneficiary, attacker]) {
  ...
  it("cliff cannot be longer than duration", async function() {
    this.start = latestTime() + duration.minutes(1); // +1 minute so it starts after contract instantiation
    this.cliff = duration.years(2) + duration.minutes(1);
    this.duration = duration.years(2);
    await TokenVesting.new(
      beneficiary,
      this.start,
      this.cliff,
      this.duration,
  ...
  it("beneficiary cannot be zero address", async function() {
    this.start = latestTime() + duration.minutes(1); // +1 minute so it starts after contract instantiation
    this.cliff = duration.years(1);
    this.duration = duration.years(2);
    await TokenVesting.new(
  ...
  it("cannot be released before cliff", async function() {
    await this.vesting
      .release(this.token.address, { from })
      .should.be.rejectedWith(EVMRevert);
  });

  describe("cannot be released by other accounts", function() {
  ...
  it("can be released by beneficiary", async function() {
    await increaseTimeTo(this.start + this.cliff + duration.weeks(1));
  ...
  it("should linearly release tokens during vesting period", async function() {
    ...
    for (let i = 1; i <= checkpoints; i++) {
      ...
      await this.vesting.release(this.token.address, { from });
      const balance = await this.token.balanceOf(beneficiary);
  ...
  it("should be revoked by owner if revocable is set", async function() {
    await this.vesting.revoke(this.token.address, {
      from: owner
    }).should.be.fulfilled;
  });
  ...
  it("should fail to be revoked by owner if revocable not set", async function() {
    ...
    await vesting
      .revoke(this.token.address, { from: owner })
      .should.be.rejectedWith(EVMRevert);
  });
